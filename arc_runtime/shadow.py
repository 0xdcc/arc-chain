...
def run_shadow_pipeline(config: ShadowPipelineConfig) -> dict[str, Any]:
    ...
    service = ArcShadowEvaluationService(bridge=quote_bridge, evaluator=economic_evaluator, ledger=ledger)
    ...
    if config.fixture_mode:
        ...
        p1_key = PoolKey(config.chain_id, "uniswap_v3", "factory", "0x" + "11" * 20, "address", "0x" + "01".zfill(40))
        p2_key = PoolKey(config.chain_id, "uniswap_v3", "factory", "0x" + "11" * 20, "address", "0x" + "02".zfill(40))

        p1 = PoolDescriptor(key=p1_key, currency0=usdc_asset, currency1=weth_asset, fee_model=FeeModel.static(500), tick_spacing=10, deployment_status="deployed")
        p2 = PoolDescriptor(key=p2_key, currency0=usdc_asset, currency1=weth_asset, fee_model=FeeModel.static(500), tick_spacing=10, deployment_status="deployed")
        ...
        # Profitable spread
        snap1 = PoolStateSnapshot(p1.key.pool_id, get_sqrt_ratio_at_tick(25), 25, 500_000_000_000_000_000, 500, 10, 1000, "0x" + "aa" * 32)
        snap2 = PoolStateSnapshot(p2.key.pool_id, get_sqrt_ratio_at_tick(-25), -25, 500_000_000_000_000_000, 500, 10, 1000, "0x" + "aa" * 32)
        ...
        raise PermissionError("Live shadow evaluation requires verified RPC endpoint authorization (G2_LIVE).")

    elapsed = time.monotonic() - start_time
    total = len(evaluated_results)
    profitable = sum(1 for r in evaluated_results if r.is_actionable or (r.breakdown and r.breakdown.economic_status == "profitable"))
